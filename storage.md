...
# 对象存储和文件存储的区别
    https://mp.weixin.qq.com/s/0fDo1igkJ7-b0f54Tsyc5A

    对象存储和文件存储共同点：都可以在多个客户端之间分享非结构化数据，但差别是语义(访问方式)，两个存储适合的场景也不尽相同。
## 对象存储适合的场景
1. 一次写，不修改，多次读。上传结束就是文件修改结束。
2. 基本没有随机访问共享数据内部位置的需求。如果有，需要先顺序完整的把数据从对象存储系统下载到本地，在本地文件系统上想顺序或者随机去读取
...
2. 支持文件内的随机访问。
3. 多客户端通过文件系统高效交换数据。
4. 文件存储支持多个客户端同时修改同一文件
5. 无论nas还是私有客户端的共享文件系统基本上都会了性能设计客户端的缓存系统，因此性能比较高。
6.文件有目录层级关系。
   7.文件存储中各种软、硬链接等复杂语义对象存储都没有对应的实现。
